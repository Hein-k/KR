...
module.exports =  function (db, container, prefixes) {
    ...
    var resource2Html = function (resource, triples, level) {
        ...
        if (level < 4) {
            ...
            db.find(resource, null, null).forEach(function (data) {
                ...
                // Add row
                rows.push(createTableRow("<a target='_blank' class='text-muted' href='" + data.predicate + "'>" + searchForLabel(data.predicate, db) + "</a>", object));
            });
        }
    ...
    resources.forEach(function (resource) {
        ...
        var subjectContainer = $('<div class="rdf2html-triple"><h4 class="subject">' + linkify(resource, prefixes, 'text-primary') + '</h4></div>')
